...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<Author> authorPredicateActive = new Predicate<Author>() {
        ...
        authors.stream().filter(authorPredicateActive).forEach(authorPrintConsumer);
        ...
        // TODO With functional interfaces declared
        Consumer<Book> activeBookPrintConsumer = new Consumer<Book>() {
            @Override
            public void accept(Book book) {
                System.out.println(book);
            }
        };
        Predicate<Book> bookPredicateActive = new Predicate<Book>() {
            public boolean test(Book book) {
                return book.published;
            }
        };
        Function<Author,Stream<Book>> getBookOfAuthor =new Function<Author, Stream<Book>>() {
        ...
        authors.stream()
                        .flatMap(getBookOfAuthor)
                                .filter(bookPredicateActive)
                                        .forEach(activeBookPrintConsumer);



        banner("Active books for all authors - lambda");
        // TODO With functional interfaces used directly
        authors.stream().flatMap(author -> author.books.stream())
        ...
        // TODO With functional interfaces declared
        ToIntFunction<Book> getPriceFunction=new ToIntFunction<Book>() {
            @Override
        ...
        // TODO With functional interfaces declared
        Predicate<Author> publishedPredicateBook=new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
                return author.active ;
            }
        };
        Predicate<Author> publishedPredicateBook2=new Predicate<Author>() {
        ...
        authors.stream().filter(publishedPredicateBook.and(publishedPredicateBook2))
